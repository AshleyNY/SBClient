...
#define findCall(address) MemoryUtils::getFuncFromCall(address)
...
public:
	...
	static void CreateHook(const char* name, uintptr_t address, void* func, void* OriginFunc) {
		if (!address) {
			Logger::ImLogF("[%s] Invalid address!", name);
			return;
		}
		if (MH_CreateHook((void*)address, func, reinterpret_cast<LPVOID*>(OriginFunc)) != MH_OK) {
			Logger::ImLogF("[%s] Failed to create hook!", name);
			return;
		}
		if (MH_EnableHook((void*)address) != MH_OK) {
			Logger::ImLogF("[%s] Failed to enable hook!\n", name);
			return;
		}
